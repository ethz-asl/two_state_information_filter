...
void MeasurementTimeline::addMeas(const std::shared_ptr<const StateBase>& meas, const TimePoint& t){
  ...
  if(!ignoreFirst_ && t<=lastProcessedTime_){
    ...
    ret = measMap_.insert(std::pair<TimePoint,std::shared_ptr<const StateBase>>(t,meas));
    if(!ret.second){
...
bool MeasurementTimeline::getMeas(const TimePoint& t, std::shared_ptr<const StateBase>& meas){
