...
int main(int argc, char** argv){
  const int L = 25;
  tsif::VioFilter<L> filter("/home/michael/workspace/generalized_information_filter/cfg/vio.cfg");
  ...
  for(int i=0;i<10000 && cam0data.good();i++){
    ...
    filter.AddMeas<6>(tsif::TimePoint(tsif::fromSec(cam0time)),meas);
    ...
    while(imu0time < cam0time){
      ...
      filter.AddMeas<2>(tsif::TimePoint(tsif::fromSec(imu0time)),measAcc);
      filter.AddMeas<3>(tsif::TimePoint(tsif::fromSec(imu0time-0.05)),measGyr); // TODO timing
    }
