...
StateBase::StateBase(const std::shared_ptr<const StateDefinition>& def): def_(def){};

StateBase::~StateBase(){};

bool StateBase::matchesDef(const std::shared_ptr<const StateDefinition>& def) const{
...
StateWrapper::StateWrapper(const std::shared_ptr<const StateDefinition>& def, const std::shared_ptr<const StateDefinition>& in): StateBase(def), in_(in){
  computeMap();
};
