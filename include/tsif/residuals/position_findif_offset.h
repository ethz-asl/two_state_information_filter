...
template<int OUT_POS, int STA_POS, int STA_VEL, int STA_ATT, int STA_ROR>
using PositionFindifBase = Residual<ElementVector<Element<Vec3,OUT_POS>>,
                                 ElementVector<Element<Vec3,STA_POS>,Element<Vec3,STA_VEL>,Element<Quat,STA_ATT>,Element<Vec3,STA_ROR>>,
                                 ElementVector<Element<Vec3,STA_POS>>,
                                 MeasEmpty>;

template<int OUT_POS, int STA_POS, int STA_VEL, int STA_ATT, int STA_ROR>
class PositionFindif: public PositionFindifBase<OUT_POS,STA_POS,STA_VEL,STA_ATT,STA_ROR>{
 public:
  typedef PositionFindifBase<OUT_POS,STA_POS,STA_VEL,STA_ATT,STA_ROR> Base;
  using Base::dt_;
  using Base::w_;
  ...
  int EvalRes(typename Output::Ref out, const typename Previous::CRef pre, const typename Current::CRef cur){
    out.template Get<OUT_POS>() = cur.template Get<STA_POS>() - pre.template Get<STA_POS>()
      - dt_*pre.template Get<STA_ATT>().toRotationMatrix()*(pre.template Get<STA_VEL>()-SSM(pre.template Get<STA_ROR>())*Vec3(0.0245, -0.04255, 0.1857));
    return 0;
  }
  int JacPre(MatRefX J, const typename Previous::CRef pre, const typename Current::CRef cur){
    ...
    J.block<3,3>(Output::Start(OUT_POS),pre.Start(STA_ATT)) = SSM(dt_*pre.template Get<STA_ATT>().toRotationMatrix()*
                                                                  (pre.template Get<STA_VEL>()-SSM(pre.template Get<STA_ROR>())*Vec3(0.0245, -0.04255, 0.1857)));
    J.block<3,3>(Output::Start(OUT_POS),pre.Start(STA_ROR)) = -dt_*pre.template Get<STA_ATT>().toRotationMatrix()*SSM(Vec3(0.0245, -0.04255, 0.1857));
    return 0;
  }
