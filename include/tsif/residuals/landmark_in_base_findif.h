...
template<int Y,//contact point innovation
         int B_V_IM,//linear velocity base to odom in base frame
...
using LandmarkInBaseFindifBase = Residual<ElementVector<Element<Vec3,Y>>,//innovation
                                    ElementVector<Element<Vec3,B_V_IM>,Element<Vec3,B_OMEGA_IB>,Element<Vec3,B_P>,Element<Vec3,B_R_BM>>,//previous state
...
template<int B_V_IM,int B_OMEGA_IB,int B_P, int B_R_BM>
class LandmarkInBaseFindif : public LandmarkInBaseFindifBase<0,B_V_IM,B_OMEGA_IB,B_P,B_R_BM> {
 public:
  //typedef and types from base for convenience
  //base type
  typedef LandmarkInBaseFindifBase<0,B_V_IM,B_OMEGA_IB,B_P,B_R_BM> Base;
  ...
  int EvalRes(typename Output::Ref out, const typename Previous::CRef pre, const typename Current::CRef cur){
   //compare measured contact points to filter state
	 out. template Get<0>()=cur.template Get<B_P>()
                          ...
                          +dt_*(pre.template Get<B_V_IM>()-SSM(pre.template Get<B_OMEGA_IB>())*pre.template Get<B_R_BM>());
   return 0;
  }
  ...
  int JacPre(MatRefX J, const typename Previous::CRef pre, const typename Current::CRef cur){
   ...
   this->template SetJacPre<0,B_V_IM>(J,pre,dt_*Mat3::Identity());
   //derivative of innovation wrt angular velocity
   this->template SetJacPre<0,B_OMEGA_IB>(J,pre,-SSM(dt_*pre.template Get<B_P>())+SSM(dt_*pre.template Get<B_R_BM>()));
