...
class LandmarkMeasurement : public ElementVector<Element<Vec3,0>>{
 public:
  //constructors with and without arguments
  LandmarkMeasurement(): ElementVector<Element<Vec3,0>>(Vec3(.0,.0,.0)) {}
  LandmarkMeasurement(const Vec3& B_Ns):ElementVector<Element<Vec3,0>>(B_Ns){}
...
using LandmarkUpdateBase = Residual<ElementVector<Element<Vec3,Y>>,//innovation
...
template<int B_P>
class LandmarkUpdate : public LandmarkUpdateBase<0,B_P> {
 public:
  //typedef and types from base for convenience
  //base type
  typedef LandmarkUpdateBase<0,B_P> Base;
  ...
  LandmarkUpdate(): Base (false,//splittable
                          false,//mergeable
                          false) {};//mandatory
