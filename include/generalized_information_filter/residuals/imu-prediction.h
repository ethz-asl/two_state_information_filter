...
class IMUPrediction : public Prediction<ElementPack<V3D, V3D, V3D, V3D, QPD>,
    ElementPack<V3D, V3D, V3D, V3D, V3D>, IMUMeas> {
 public:
  IMUPrediction()
      : mtPrediction( { "pos", "vel", "gyb", "acb", "att" }, { "pos", "vel",
                         "gyb", "acb", "att" }),
        g_(0, 0, -9.81) {
    dt_ = 0.1;
  }
  ...
  void evalPredictionImpl(V3D& posPos, V3D& velPos, V3D& gybPos, V3D& acbPos,
                          ...
                          const QPD& attPre, const V3D& posNoi,
                          const V3D& velNoi, const V3D& gybNoi,
                          const V3D& acbNoi, const V3D& attNoi) const {
    ...
    velPos = (M3D::Identity() - gSM(dOmega)) * velPre
        + dt_ * (acc + attPre.inverseRotate(g_));
    gybPos = gybPre + gybNoi * sqrt(dt_);
    acbPos = acbPre + acbNoi * sqrt(dt_);
    attPos = attPre * dQ;
  }
  void jacPrePredictionImpl(MXD& J, const V3D& posPre, const V3D& velPre,
                            const V3D& gybPre, const V3D& acbPre,
                            const QPD& attPre, const V3D& posNoi,
                            const V3D& velNoi, const V3D& gybNoi,
                            const V3D& acbNoi, const V3D& attNoi) const {
    ...
    setJacBlockPre<VEL, ATT>(J,
                             dt_ * MPD(attPre).matrix().transpose() * gSM(g_));
  ...
  void jacNoiPredictionImpl(MXD& J, const V3D& posPre, const V3D& velPre,
                            const V3D& gybPre, const V3D& acbPre,
                            const QPD& attPre, const V3D& posNoi,
                            const V3D& velNoi, const V3D& gybNoi,
                            const V3D& acbNoi, const V3D& attNoi) const {
    ...
    setJacBlockNoi<ATT, ATT>(J,
                             sqrt(dt_) * MPD(attPre).matrix() * Lmat(dOmega));
  }
