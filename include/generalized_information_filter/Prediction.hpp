...
class Prediction<ElementPack<Sta...>,ElementPack<Noi...>,Meas>:
 ...
 protected:
  ...
  void _evalPredictionImpl(const std::shared_ptr<StateBase>& pos, const Sta&... pre, const Noi&... noi, Ts&... elements) const{
    ...
    _evalPredictionImpl(pos, pre..., noi..., elements..., std::dynamic_pointer_cast<Element<mtElementType>>(pos->getElement(innerIndex))->get());
  }
  template<typename... Ts, typename std::enable_if<(sizeof...(Ts)==ElementPack<Sta...>::n_)>::type* = nullptr>
  void _evalPredictionImpl(const std::shared_ptr<StateBase>& pos, const Sta&... pre, const Noi&... noi, Ts&... elements) const{
  ...
  inline void computeInnovation(Sta&... res, const Sta&... pos, const std::shared_ptr<const StateBase>& prediction) const{
    ...
    ElementTraits<mtElementType>::boxminus(std::dynamic_pointer_cast<const Element<mtElementType>>(prediction->getElement(i))->get(),std::get<i>(std::forward_as_tuple(pos...)),vec);
    ElementTraits<mtElementType>::boxplus(ElementTraits<mtElementType>::identity(),vec,std::get<i>(std::forward_as_tuple(res...))); // TODO: make more efficient (could be done directly on boxminus, but then jacobian becomes more annoying)
  ...
  void computePosJacobian(MXD& J, const std::shared_ptr<StateBase>& prediction, const Sta&... pos) const{
    assert(prediction->matchesDef(this->posDefinition()));
    typedef typename std::tuple_element<i,typename ElementPack<Sta...>::mtTuple>::type mtElementType;
    Eigen::Matrix<double,ElementTraits<mtElementType>::d_,1> vec;
    ElementTraits<mtElementType>::boxminus(std::dynamic_pointer_cast<const Element<mtElementType>>(prediction->getElement(i))->get(),std::get<i>(std::forward_as_tuple(pos...)),vec);
    J.template block<ElementTraits<mtElementType>::d_,ElementTraits<mtElementType>::d_>(j,j) = ElementTraits<mtElementType>::boxplusJacVec(ElementTraits<mtElementType>::identity(),vec)
                                                                                              *ElementTraits<mtElementType>::boxminusJacRef(std::dynamic_pointer_cast<const Element<mtElementType>>(prediction->getElement(i))->get(),std::get<i>(std::forward_as_tuple(pos...)));
